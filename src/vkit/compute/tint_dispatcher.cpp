...
TintDispatcher::TintDispatcher(const graphics::GfxDevice& device,
    ...
    : device_{device},
      ...
      sub_{jobBus.subscribe(
          [this](core::events::TintJobRequest& req) { onRequest(req); })} {
  setLayout_ = std::make_unique<dsl::ImageOperatorSetLayout>(device.get());
  pipelineLayout_ = std::make_unique<pl::TintPipelineLayout>(
      device.get(), std::forward_as_tuple(*setLayout_));

  graphics::SpirVShaderModule shader(
      device.get(), shaders::shaderPath(shaders::kOperatorsTintShaderPath));
...
void TintDispatcher::onRequest(core::events::TintJobRequest& req) {
  ...
  vk::Sampler actual_sampler =
      input_gfx->getSampler() ? input_gfx->getSampler() : *sampler_;

  vk::DescriptorImageInfo in_info{actual_sampler, input_gfx->getImageView(),
  ...
  std::array<vk::WriteDescriptorSet, 3> writes{
      vk::WriteDescriptorSet{ds, dsl::ImageOperatorSetLayout::kInImageBinding,
                             0, 1, vk::DescriptorType::eCombinedImageSampler,
                             &in_info},
      vk::WriteDescriptorSet{
          ds, dsl::ImageOperatorSetLayout::kOutImageF32Binding, 0, 1,
          vk::DescriptorType::eStorageImage, &f32_info},
      vk::WriteDescriptorSet{
          ds, dsl::ImageOperatorSetLayout::kOutImageUnormBinding, 0, 1,
          vk::DescriptorType::eStorageImage, &unorm_info},
  };
  ...
  inFlightJobs_.push_back(InFlightTintJob{
...
void TintDispatcher::update() {
  std::vector<core::events::TintJobResult> completed;
