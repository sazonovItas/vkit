...
HeightMapDispatcher::HeightMapDispatcher(
    const graphics::GfxDevice& device, core::events::HeightMapJobBus& jobBus,
    ...
    : device_{device},
      ...
      sub_{jobBus.subscribe(
          [this](core::events::HeightMapJobRequest& req) { onRequest(req); })} {
  setLayout_ = std::make_unique<dsl::ImageOperatorSetLayout>(device.get());
  pipelineLayout_ = std::make_unique<pl::HeightMapPipelineLayout>(
...
void HeightMapDispatcher::onRequest(core::events::HeightMapJobRequest& req) {
  ...
  inFlightJobs_.push_back(InFlightHeightMapJob{
...
void HeightMapDispatcher::update() {
  std::vector<core::events::HeightMapJobResult> completed;
