...
NormalMapDispatcher::NormalMapDispatcher(
    const graphics::GfxDevice& device, core::events::NormalMapJobBus& jobBus,
    ...
    : device_{device},
      ...
      sub_{jobBus.subscribe(
          [this](core::events::NormalMapJobRequest& req) { onRequest(req); })} {
  setLayout_ = std::make_unique<dsl::ImageOperatorSetLayout>(device.get());
  pipelineLayout_ = std::make_unique<pl::NormalMapPipelineLayout>(
      device.get(), std::forward_as_tuple(*setLayout_));

  graphics::SpirVShaderModule shader(
      device.get(), asset::assetPath(asset::kOperatorsNormalMapShaderPath));
...
void NormalMapDispatcher::onRequest(core::events::NormalMapJobRequest& req) {
  ...
  inFlightJobs_.push_back(InFlightNormalMapJob{
...
void NormalMapDispatcher::update() {
  std::vector<core::events::NormalMapJobResult> completed;
