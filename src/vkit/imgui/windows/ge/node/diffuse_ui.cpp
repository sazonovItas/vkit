...
using workflow::node::mat::DiffuseNode;

static const ImVec4 kPinColorMaterial{0.8F, 0.3F, 0.8F, 1.0F};

auto DiffuseNodeUI::spawnNode(controller::WorkflowController* controller)
    -> workflow::WorkflowNode* {
  return controller->createDiffuseNode("Diffuse");
}

void DiffuseNodeUI::drawCanvas(workflow::WorkflowNode* node) {
  auto* n = static_cast<DiffuseNode*>(node);
...
void DiffuseNodeUI::drawInspector(workflow::WorkflowNode* node) {
  auto* n = static_cast<DiffuseNode*>(node);
  bool changed = false;

  ImGui::TextDisabled("Type: Diffuse Material");
  ImGui::Separator();
