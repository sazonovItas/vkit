...
NormalMapNode::NormalMapNode(std::string_view name,
    ...
    : WorkflowNode(name),
      ...
      resultSub_{
          resultBus.subscribe([this](core::events::NormalMapJobResult& ev) {
  ...
  inImage_ = addInputPin(pinKeyType(PinType::kColorTexture2D), "Height Map");
  outImageF32_ =
      addOutputPin(pinKeyType(PinType::kFloatTexture2D), "Normal (F32)");
  outColor_ =
      addOutputPin(pinKeyType(PinType::kColorTexture2D), "Normal (Color)");
}
...
void NormalMapNode::execute() {
  ...
  jobBus_.queueMessage(core::events::NormalMapJobRequest{
...
void NormalMapNode::setParams(NormalMapParams params) {
  params_ = params;
  pendingRequestId_ = 0;
  markStale();
}
