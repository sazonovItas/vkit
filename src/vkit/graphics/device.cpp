...
QueueFamilies::QueueFamilies(vk::PhysicalDevice physicalDevice,
  ...
  graphicsPresent = [&] -> std::uint32_t {
    for (const auto &[idx, props] :
      ...
      if (util::contains(props.queueFlags, vk::QueueFlagBits::eGraphics) &&
          physicalDevice.getSurfaceSupportKHR(idx, surface) == vk::True) {
        return idx;
      }
    }
...
Queues::Queues(vk::Device device, const QueueFamilies &queueFamilies) noexcept
