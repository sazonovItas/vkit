...
inline ImRect GetContainingRectForCubicBezier(const CubicBezier& cb)
{
    ...
    const float hover_distance = GImNodes->Style.LinkHoverDistance * GImNodes->EditorCtx->Zoom;
    ...
    const ImNodesAttributeType type)
{
    ...
    const float pin_offset = GImNodes->Style.PinOffset * GImNodes->EditorCtx->Zoom;
    const float x = type == ImNodesAttributeType_Input
                        ? (node_rect.Min.x - pin_offset)
                        : (node_rect.Max.x + pin_offset);
    ...
    if (GImNodes->LeftMouseDragging)
    {
        ...
        const ImVec2 origin = SnapOriginToGrid(
            (GImNodes->MousePos - GImNodes->CanvasOriginScreenSpace - editor.Panning +
             editor.PrimaryNodeOffset) / editor.Zoom);
...
void ClickInteractionUpdate(ImNodesEditorContext& editor)
{
    ...
    case ImNodesClickInteractionType_LinkCreation:
    {
        ...
        GImNodes->CanvasDrawList->AddBezierCubic(
            ...
            GImNodes->Style.LinkThickness * GImNodes->EditorCtx->Zoom,
    ...
    const ImVector<int>&           occluded_pin_indices)
{
    ...
    const float pin_hover_radius = GImNodes->Style.PinHoverRadius * GImNodes->EditorCtx->Zoom;
    const float hover_radius_sqr = pin_hover_radius * pin_hover_radius;
    ...
    for (int idx = 0; idx < links.Pool.Size; ++idx)
    {
        ...
        // The distance test
        {
            ...
            if (link_rect.Contains(GImNodes->MousePos))
            {
                ...
                if (distance < GImNodes->Style.LinkHoverDistance * GImNodes->EditorCtx->Zoom && distance < smallest_distance)
                {
...
{
    const float  zoom = GImNodes->EditorCtx->Zoom;
    const ImVec2 title_bar_height =
        ImVec2(0.f, node.TitleBarContentRect.GetHeight() / zoom + 2.0f * node.LayoutStyle.Padding.y);
...
inline ImRect GetNodeTitleRect(const ImNodeData& node)
{
    ...
    expanded_title_rect.Expand(node.LayoutStyle.Padding * zoom);
...
void DrawGrid(ImNodesEditorContext& editor, const ImVec2& canvas_size)
{
    ...
    const float grid_spacing = GImNodes->Style.GridSpacing * editor.Zoom;
...
void DrawPinShape(const ImVec2& pin_pos, const ImPinData& pin, const ImU32 pin_color)
{
    ...
    const float      z = GImNodes->EditorCtx->Zoom;
    ...
    case ImNodesPinShape_Circle:
    {
        GImNodes->CanvasDrawList->AddCircle(
            ...
            pin_color,
            CIRCLE_NUM_SEGMENTS,
            GImNodes->Style.PinLineThickness * z);
    }
    break;
    case ImNodesPinShape_CircleFilled:
    {
        GImNodes->CanvasDrawList->AddCircleFilled(
            pin_pos, GImNodes->Style.PinCircleRadius * z, pin_color, CIRCLE_NUM_SEGMENTS);
    }
    break;
    case ImNodesPinShape_Quad:
    {
        const QuadOffsets offset = CalculateQuadOffsets(GImNodes->Style.PinQuadSideLength * z);
    ...
    case ImNodesPinShape_Triangle:
    {
        const TriangleOffsets offset =
            CalculateTriangleOffsets(GImNodes->Style.PinTriangleSideLength * z);
        GImNodes->CanvasDrawList->AddTriangle(
            pin_pos + offset.TopLeft,
            pin_pos + offset.BottomLeft,
            pin_pos + offset.Right,
            pin_color,
            2.f * GImNodes->Style.PinLineThickness * z);
    }
    break;
...
void BeginNodeEditor()
{
    ...
    ImGui::BeginGroup();
    {
...

        {
            ...
            if (scroll != 0.f && MouseInCanvas())
            {
                const float zoom_factor = scroll > 0.f ? 1.1f : (1.0f / 1.1f);
                const float old_zoom    = editor.Zoom;
                editor.Zoom             = ImClamp(editor.Zoom * zoom_factor, 0.1f, 4.0f);
                const ImVec2 m          = GImNodes->MousePos - GImNodes->CanvasOriginScreenSpace;
                editor.Panning          = m - (m - editor.Panning) * (editor.Zoom / old_zoom);
            }
